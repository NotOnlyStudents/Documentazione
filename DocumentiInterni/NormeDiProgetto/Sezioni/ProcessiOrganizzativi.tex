...
I processi organizzativi comprendono tutti i processi utilizzati per definire e gestire una struttura costituita dal personale e dai processi del ciclo di vita del \glo{prodotto}, al fine di migliorarne continuamente la struttura e i processi stessi. 
...
Come stabilito dallo \glo{standard} \glo{ISO/IEC 12207:1995}, comprende le \glo{attività} e i compiti generici per la gestione dei rispettivi processi.

\subsubsection{Descrizione della sezione}
La sezione definisce le norme stabilite dal gruppo per la pianificazione e il coordinamento del lavoro dei componenti del \glo{team}.
...
Per permettere di gestire i compiti in modo agevole è stato scelto di utilizzare l'\glo{Issue Tracking System} fornito da \glo{\textit{GitHub}}: il \glo{repository} usato dal gruppo per il \glo{versionamento} del progetto.
...
\myparagraph{Metriche}\label{MPianificazione}
...
I membri utilizzano per le comunicazioni interne il gruppo \glo{\textit{Telegram}} appositamente creato. Per tutte le comunicazioni importanti viene preferito utilizzare \textit{Zoom} effettuando quindi delle video-chiamate che permettono un feedback immediato.
...
\begin{itemize}
	...
	\item \textbf{\textit{Telegram}:} strumento di messaggistica istantanea basato su \glo{cloud}, utilizzato dal gruppo \Gruppo\ per le comunicazioni tra i vari membri;
	\item \textbf{\textit{Gmail}:} strumento \glo{open source} di posta elettronica, utilizzato per gestire la casella e-mail del gruppo;
	...
	\item \textbf{\textit{GanttProject}:} utilizzato per la pianificazione e la realizzazione dei \glo{diagrammi di Gantt}.
\end{itemize}
...
All'interno del \PdQv{}\ vengono inseriti gli obiettivi e le metriche di \glo{qualità} per valutare l'andamento delle attività di ogni processo. Qualora si presentassero problemi, il membro del gruppo deve avvertire tempestivamente gli altri componenti del \glo{team} e si procederà ad una eventuale modifica di quanto pianificato. Periodicamente vengono calcolate le metriche di qualità previste.

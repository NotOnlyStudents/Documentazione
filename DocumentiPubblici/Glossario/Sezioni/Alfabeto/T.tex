...
\TermineGlossario{Team/Teamwork}
\DefinizioneGlossario{
	Lavoro collaborativo tra più membri che punta a raggiungere un obiettivo comune in modo efficace ed efficiente.
}
\TermineGlossario{Test}
\DefinizioneGlossario{
	Operazione effettuata durante il periodo di sviluppo in cui il \glo{prodotto} viene controllato per individuare eventuali problemi o malfunzionamenti.
}
...
\DefinizioneGlossario{
	Server web \glo{open source} per implementare le specifiche \textit{JavaServer Pages} e \textit{Java Servlet}, fornendo quindi una piattaforma software per l'esecuzione di applicazioni web sviluppate in linguaggio \glo{\textit{Java}}.
}
\TermineGlossario{Typescript}
\DefinizioneGlossario{
	Il linguaggio di programmazione open source sviluppato da Microsoft. Estende la sintassi di \glo{JavaScript} in modo che qualunque programma scritto in \glo{JavaScript} sia anche in grado di funzionare con \glo{TypeScript} senza nessuna modifica.
}
\TermineGlossario{Typescript-eslint}
\DefinizioneGlossario{
	Strumento di analisi del codice statico per identificare i modelli problematici trovati nel codice \glo{\textit{TypeScript}}.
}